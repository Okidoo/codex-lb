...
MODEL_SOURCE_KIND_ZAI_CODING_PLAN = "zai_coding_plan"
...
_ZAI_MODEL_RAW: dict[str, JsonValue] = {"provider": "zai", "max_output_tokens": 131_072}
_ZAI_GLM_52_COMPATIBILITY_SLUG = next(
    alias for alias, target in ZAI_MODEL_ALIASES.items() if target == "glm-5.2"
)
_LOCAL_ZAI_MODELS: dict[str, UpstreamModel] = {
    "glm-5.2": _bootstrap_model(
        "glm-5.2",
        "GLM-5.2",
        ...
        raw=_ZAI_MODEL_RAW,
        source_kind=MODEL_SOURCE_KIND_ZAI_CODING_PLAN,
    ),
    _ZAI_GLM_52_COMPATIBILITY_SLUG: _bootstrap_model(
        ...
        *,
        ...
        async with self._lock:
            ...
            try:
                ...
                for slug, model in _LOCAL_ZAI_MODELS.items():
                    models[slug] = model
