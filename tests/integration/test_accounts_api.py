...
from app.db.models import Account, AccountStatus, ZaiCredential
...
async def test_create_list_and_delete_zai_account_stores_encrypted_credential(async_client):
    ...
    response = await async_client.post(
        "/api/accounts/zai",
        json={"apiKey": api_key, "label": "Coding GLM", "baseUrl": base_url},
    )
    ...
    assert created["label"] == "Coding GLM"
    assert created["planType"] == "zai"
    ...
    assert account["provider"] == "zai"
    assert account["alias"] == "Coding GLM"
    ...
    delete_response = await async_client.delete(f"/api/accounts/{account_id}")
    assert delete_response.status_code == 200

    async with SessionLocal() as session:
        assert await session.get(Account, account_id) is None
        remaining = await session.scalar(select(ZaiCredential).where(ZaiCredential.account_id == account_id))
